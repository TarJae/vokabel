...
`vokabel` provides versatile functions customizing your questions beyond the predefined question types. By using `create_question()` and `create_question_raw()` functions, you can tailor your quizzes to suit a wide array of educational scenarios and learning styles.
...
For example, a custom plot can be included in the question by passing a plot to `shiny::renderPlot()` and wrapping the entire prompt using `htmltools::tagList()`. Note that some HTML functions will not print correctly so using `vokabel::preview_app()` is recommended.
...
For example, you could use `shiny::dateInput()` to specify a date input. Note the use of `shiny::NS('quiz')('answers')` to define the `inputId`. This is required for `vokabel` to find the user's answer. 
